...
| `MAX_MCP_OUTPUT_TOKENS` | Max output tokens | 25000 |

## Hooks with MCP

Target MCP tools in hooks using `mcp__<server>__<tool>` pattern:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "log-github-operation.sh"
          }
        ]
      }
    ]
...
### Use in Headless
```bash
claude -p "Query" --mcp-config ./config.json --allowedTools "mcp__server__*"
```

### Strict Mode
```bash
claude --strict-mcp-config --mcp-config ./only-these.json
```
