...
|------|-------------|----------|
| `default` | Prompt on first use | General development |
| `acceptEdits` | Auto-accept file edits | Focused coding |
| `plan` | Read-only (no modify/execute) | Code review |
| `bypassPermissions` | Skip all prompts | Controlled automation |
...
| `/` | Relative to settings | `Edit(/src/**)` |
| (none) | Relative to cwd | `Read(.env)` |
...
# Plan mode
claude -p "Review" --permission-mode plan
```
