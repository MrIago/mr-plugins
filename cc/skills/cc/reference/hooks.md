...
| Event | When | Has Matcher | Can Block |
|-------|------|-------------|-----------|
| **PreToolUse** | Before tool executes | Yes | Yes |
| **PermissionRequest** | Permission dialog shown | Yes | Yes |
| **PostToolUse** | After tool completes | Yes | Partial |
| **UserPromptSubmit** | Prompt submitted | No | Yes |
| **Stop** | Claude finishes | No | Yes |
| **SubagentStop** | Subagent finishes | No | Yes |
| **PreCompact** | Before compact | Yes | No |
...
BLOCKED = [r"rm\s+-rf\s+/", r"sudo\s+rm", r">\s*/dev/sd", r"mkfs\.", r"dd\s+if=.*of=/dev"]
...
BLOCKED = ['.env', 'secrets', '.pem', '.key', 'credentials']

data = json.load(sys.stdin)
path = data.get("tool_input", {}).get("file_path", "")

if any(p in path for p in BLOCKED):
    print(f"BLOCKED: Cannot access {path}", file=sys.stderr)
    sys.exit(2)
sys.exit(0)
```
...
```bash
#!/bin/bash
cat >> "$CLAUDE_PROJECT_DIR"/.claude/operations.log
echo "---" >> "$CLAUDE_PROJECT_DIR"/.claude/operations.log
exit 0
```
...
COMMITS=$(git log --oneline -5 2>/dev/null || echo "No git")
TODOS=$(grep -r "TODO\|FIXME" --include="*.ts" 2>/dev/null | wc -l)
...
### Add Context
```python
output = {"hookSpecificOutput": {"hookEventName": "PostToolUse", "additionalContext": "Extra info"}}
print(json.dumps(output))
sys.exit(0)
```

### Force Continue (Stop Hook)
```python
output = {"decision": "block", "reason": "Not done yet"}
print(json.dumps(output))
sys.exit(0)
```
