...
```bash
# Step 1: Plan
SESSION=$(claude -p "Plan implementation" \
...
claude -p "Run tests and fix failures" \
  ...
  --max-turns 10 \
  --output-format json \
  --append-system-prompt "Only fix critical issues"
```

### Security Review

```bash
git diff HEAD~1 | claude -p "Review for security vulnerabilities" \
  --tools "Read,Grep" \
