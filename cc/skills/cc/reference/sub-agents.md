...
claude = Claude(agents=[security_agent])
result = claude.run("Audit src/auth/ for vulnerabilities")
```
...
const claude = new Claude({ agents: [securityAgent] });
const result = await claude.run('Audit src/auth/');
```
...
| Wrong tools | Verify tools array in YAML |
| Timeout | Use `run_in_background` + `TaskOutput` |
| Context missing | Pass context explicitly in prompt |
| Agent too slow | Use haiku model for simple tasks |
...
- `Plan` - Planning, read-only, sonnet
