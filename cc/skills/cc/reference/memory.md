...
```
1. Enterprise Policy    /etc/claude-code/CLAUDE.md       (IT enforced)
         ↓
2. Local Project       ./CLAUDE.local.md                 (personal, gitignored)
         ↓
3. Path-based Rules    ./.claude/rules/*.md              (path-specific)
         ↓
4. Root Project        ./CLAUDE.md                       (team shared)
         ↓
5. User Global         ~/.claude/CLAUDE.md               (personal global)
         ↓
...
| What | When Loaded |
|------|-------------|
| CLAUDE.md | Always (keep minimal!) |
| CLAUDE.local.md | Always |
| Path-based rules | Only when file matches pattern |
| Skills | Only when invoked |
| Commands | Only when invoked |

**Token budget:**
- Input: up to 200k tokens
- Output: up to 16k (Sonnet), 32k (Opus)

## Quick Reference

| File | Scope | Committed |
|------|-------|-----------|
| `/etc/claude-code/CLAUDE.md` | Machine (enterprise) | N/A |
| `~/.claude/CLAUDE.md` | User (all projects) | No |
| `~/.claude/rules/*.md` | User rules | No |
| `./CLAUDE.md` | Project (team) | Yes |
| `./CLAUDE.local.md` | Project (personal) | No |
| `./.claude/rules/*.md` | Project rules | Yes |

### Golden Rules

1. **Declarative > Imperative** - State what, not how
2. **Specific > Vague** - "2 spaces" not "proper formatting"
3. **Structured > Prose** - Use headings and bullets
4. **Path-based > Conditionals** - Split by path, not if/else
5. **Minimal root > Everything loaded** - Use rules/ and imports
6. **Team in repo > Personal in local** - Separate concerns
