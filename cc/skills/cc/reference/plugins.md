...
**Symptom**: Claude runs commands manually instead of using hook-injected instructions.

**Check 1**: Verify `hooks.json` format. Plugins require the `hooks` wrapper at the top level:
...
**Check 2**: `hooks/hooks.json` is auto-loaded. Do NOT add reference in `plugin.json`:
...
// CORRECT - hooks/hooks.json auto-loaded
{
  "name": "my-plugin"
}
```

Only use `"hooks"` field for non-standard paths (e.g., `"hooks": "./custom/my-hooks.json"`).

### Hook runs but Claude ignores instructions

**Symptom**: State file is created but Claude does its own thing.

**Solution**: For `UserPromptSubmit` hooks, use JSON structured output with `additionalContext`:

```bash
# Plain text output may be ignored by Claude
echo "Do this task"

# Use JSON for reliable context injection
cat << EOF
{
  "hookSpecificOutput": {
...
Add a version field to state file for debugging:
...
```bash
cat ~/.claude/plugins/marketplaces/<marketplace-name>/<plugin-name>/.claude-plugin/plugin.json
```

**Force update from marketplace**:
...
```bash
ls -la scripts/
# Must be executable: -rwx--x--x
chmod +x scripts/*.sh
```

**Check shebang** (first line of script):
...
| `${CLAUDE_PROJECT_DIR}` | Project root where Claude Code runs |

**Important**: Use `CLAUDE_PROJECT_DIR` for git operations - hooks run from the user's project, not the plugin folder.

## Headless Mode Compatibility

| Feature | Works in `-p` | Notes |
|---------|--------------|-------|
| LSP servers from plugins | Yes | Auto-discovered |
| MCP servers from plugins | Yes | Auto-connected |
| Skills from plugins | Yes | Auto-invoked |
| Custom agents | Yes | Via Task tool |
| Slash commands | No | Requires interactive |
| `/plugin install` | No | Interactive command |
| OAuth authentication | No | Requires user interaction |

### Pre-Installing for Containers

```dockerfile
# Install Claude Code
RUN curl -fsSL https://claude.ai/install.sh | sh

# Pre-configure plugins via settings
